...
const AdminProducts: React.FC = () => {
  ...
  const [editingProduct, setEditingProduct] = useState<Product | null | undefined>(undefined); // undefined = list view, null = new mode, obj = edit mode
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [prods, cats] = await Promise.all([mockApi.getProducts(), mockApi.getCategories()]);
      setProducts(prods);
      setCategories(cats);
    } catch (e) {
      console.error(e);
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-64">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 border rounded focus:ring-2 focus:ring-emerald-500"
            />
          </div>
          <button 
        ...
        <table className="w-full text-left border-collapse">
          <thead className="bg-emerald-50 text-emerald-700 text-sm uppercase">
            <tr>
              ...
              <th className="p-4 font-semibold">Giá từ</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr><td colSpan={5} className="p-8 text-center text-gray-500">Đang tải...</td></tr>
               ...
               <tr><td colSpan={5} className="p-8 text-center text-gray-500">Không tìm thấy sản phẩm nào.</td></tr>
            ) : (
                ...
                return (
                  <tr key={product.id} className="hover:bg-emerald-50/50 transition-colors">
                    ...
                    <td className="p-4 text-sm font-medium">
                      {minPrice > 0 ? new Intl.NumberFormat('vi-VN').format(minPrice) : '0'} 
                    ...
                    <td className="p-4">
                      <span className={`px-2 py-1 text-xs rounded-full font-medium ${
                        ...
                        product.status === 'draft' ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600'
                      }`}>
                        {product.status === 'published' ? 'Đang hiện' : product.status === 'draft' ? 'Nháp' : 'Đã ẩn'}
                      </span>
                    </td>
