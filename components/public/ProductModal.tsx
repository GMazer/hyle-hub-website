...
import { X, CheckCircle2, Copy, MessageCircle, Info } from 'lucide-react';
...
const ProductModal: React.FC<ProductModalProps> = ({ product, siteConfig, isOpen, onClose }) => {
  ...
  const contactLink = siteConfig.contactInfo.email ? `mailto:${siteConfig.contactInfo.email}?subject=Đặt hàng ${product.name}` : '#';

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      <div className="absolute inset-0 bg-emerald-900/40 backdrop-blur-sm" onClick={onClose} />
      <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-white dark:bg-gray-900 rounded-3xl shadow-2xl animate-in fade-in zoom-in duration-200 transition-colors border border-emerald-100 dark:border-gray-800">
        ...
        <div className="flex flex-col md:flex-row h-full">
          ...
          <div className="w-full md:w-5/12 bg-emerald-50/50 dark:bg-gray-800/50 p-6 md:p-8 flex items-center justify-center">
            <div className="w-full aspect-square rounded-2xl overflow-hidden shadow-lg border-4 border-white dark:border-gray-700">
              <img 
                src={product.thumbnailUrl} 
                alt={product.name} 
                className="w-full h-full object-contain bg-white dark:bg-gray-800 p-4"
              />
            </div>
          </div>
          ...
          <div className="w-full md:w-7/12 p-6 md:p-8 flex flex-col">
                      ...
                      >
                        ...
                        <div className="text-right">
                          <div className="font-extrabold text-lg text-emerald-600 dark:text-emerald-400">
                            {new Intl.NumberFormat('vi-VN').format(opt.price)}{opt.currency}
                          </div>
            ...
            {/* Actions */}
            <div className="mt-6 flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-100 dark:border-gray-800">
              <a 
                href={contactLink}
                target="_blank"
                rel="noreferrer"
